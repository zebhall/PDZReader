...
def main():
	...
	with open('temps.txt','x') as fw:
		fw.write("num,name,time,detectortemp,ambienttemp,nosetemp,rawcountstotal,validcountstotal")
		fw.write("\n")
		for i in range(len(pdz_fnames)):
			fw.write(f"{i+1},{pdz_fnames[i]},{str(datetimes[i])},{str(detectortemps[i])},{str(ambienttemps[i])},{str(nosetemps[i])},{str(rawcounttotals[i])},{str(validcounttotals[i])}")
			fw.write("\n")
